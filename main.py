...
from visualize import CaseVisualizer
...
def main():
    randomCaseGenerator = RandomCaseGenerator()
    astar = AStarSolver()
    Visualizer = CaseVisualizer()

    # Static case with predefined data
    static_case = DeliveryCase(
        casetime=datetime.now(),
        drones=DRONES_DATA,
        packages=DELIVERIES_DATA,
        noflyzones=NO_FLY_ZONES_DATA
    )
    static_case.case_summary()
    Visualizer.visualize(astar.solve(static_case))
    
    # 5 drone, 20 package, 2 no-fly zone
    drones1, packages1, noflyzones1 = randomCaseGenerator.get_random_data(5, 20, 2)
    random_case1 = DeliveryCase(
        casetime=datetime.now(),
        drones=drones1,
        packages=packages1,
        noflyzones=noflyzones1
    )
    ...
    random_case2 = DeliveryCase(
        ...
        drones=drones2,
        packages=packages2,
        noflyzones=noflyzones2
    )
    random_case2.case_summary()
    Visualizer.visualize(astar.solve(random_case2))
